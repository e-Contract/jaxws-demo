...
/**
 * Validate results corresponding to the processing of a Signature, EncryptedKey
 * or EncryptedData structure against an AlgorithmSuite policy.
 */
public class ECDSAAlgorithmSuitePolicyValidator extends AbstractSecurityPolicyValidator {

    private static final Logger LOGGER = LoggerFactory.getLogger(ECDSAAlgorithmSuitePolicyValidator.class);

    /**
     * Return true if this SecurityPolicyValidator implementation is capable of
     * validating a policy defined by the AssertionInfo parameter
     */
    ...
    private boolean checkPublicKeyLength(
        ...
        if (publicKey instanceof RSAPublicKey) {
            ...
            int length = ecPublicKey.getParams().getCurve().getField().getFieldSize();
            LOGGER.debug("field size: {}", length);
            if (length < algorithmSuiteType.getMinimumAsymmetricKeyLength()
                    || length > algorithmSuiteType.getMaximumAsymmetricKeyLength()) {
                ai.setNotAsserted(
                        "The asymmetric key length does not match the requirement"
